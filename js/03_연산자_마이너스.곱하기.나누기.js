...
/*
 ...
 . 이 항목을 선택하겠다.
  getE1ementById : 함수 안에서 메서드 기능으로 document를 사용해야만
                    호출할 수 있는 기능으로
...
("아이디명칭") : 요소 태그 내부에서 <input id="아이디명칭">으로 존재하는
   ...
   .value       바라만 보는 상태에서input 내부에 클라이언트(소비자)가 작성한 값을
                가져오기 위애서 value 를 사용
                value 로 마찬가지로 하위 기능이기 때문에
                단독으로 사용 불가능
...
개발자, 소비자가 작성한 값을 ㅅ주고 받는 역할을 하는 변수명칭은 소문자로 작성
...
function minusFn() {
  // valuse 의 값은 반드시 function 기능 내부에서 가져오기를 작성해야함
  // 왜냐하면 HTML 화면이 보여지는 순간 value 의 기본 값은 0
  // 우리가 원하는 value 값은 소비자가 작성한 값을 원하는 것이기 때문에
  // 마이너스 버튼을 클리하기 전까지 소비자가 value 값을 작성할 시간을 주어야함
  // 소비자가 작성을 모두 완료했다면 원하는 계산기 버튼을 클릭해서 계산결과값을 확인할 수있도록 해야한다.
